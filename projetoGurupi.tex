...
Um método de inferência Bayesiana depende de um conhecimento a priori dos dados que se deseja estimar.

A distribuição a priori da imagem original é dada por:
...
A solução proposta no artigo original é marginalizar $\mathbf{x}$ em (\ref{eq:posterior0}) para que ele não seja mais uma variável na função. O resultado disto é a seguinte distribuição:
