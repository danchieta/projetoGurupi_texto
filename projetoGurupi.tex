...
\begin{gather}
	...
	L'(\mathbf{x}) = -\frac{1}{2} \left\{ \sum^K_{k=1} = \mathbf{W}^{(k)T} (\mathbf{y}^{(k)} - \mathbf{W}^{(k)}\mathbf{x}) - \mathbf{Z}^T_x\mathbf{x} \right\} \\
	L''(\mathbf{x}) =  -\frac{1}{2} \left\{\sum^K_{k=1} (\mathbf{W}^{(k)}T\mathbf{W}^{(k)}) - \mathbf{Z}_x \right\}
\end{gather}
