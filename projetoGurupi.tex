\documentclass[12pt,openright,oneside,a4paper,eglish,brazil]{abntex2}
...
Sabendo disso, a relação entre uma imagem de alta resolução $\mathbf{x}$ e uma imagem degradada $\mathbf{y}^{(k)}$ é resumida em (\ref{eq:degradation}). Onde $k = 1,2,...,K$, e $K$ é o número total de quadros obtido a partie da cena de alta resolução $\mathbf{x}$.

\begin{equation}
  \label{eq:degradation}
  \mathbf{y} = \mathbf{Wx} + \mathbf{n}
\end{equation}
