...
Baseado no modelo de observação apresentado na equação \ref{eq:degradation} podemos dizer que
\begin{equation}
...
\begin{equation}
	...
	L\{\mathbf{y}^{(k)}\} = -\beta (\mathbf{y}^{(k)} - \mathbf{W}^{(k)}\hat{\mathbf{x}})^T \mathbf{W}^{(k)} 
\end{equation}
