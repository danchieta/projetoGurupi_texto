...
\begin{equation}
	L\{\mathbf{y}^{(k)}\} = -\frac{\beta}{2}\|\mathbf{y}^{(k)} - \mathbf{W}^{(k)}\hat{\mathbf{x}}\|^2.
\end{equation}

Teoricamente, a melhor estimativa ($\hat{\mathbf{x}}$) da imagem de alta resolução é a que maximiza a função de verossimilhança acima. Isso é demonstrado em (\ref{eq:maxlhood}).

\begin{equation}
	\label{eq:maxlhood}
	\mathbf{y}^{(k)}_{ML} = \argmax_{\hat{\mathbf{x}}}
	\left[ -\frac{\beta}{2}\|\mathbf{y}^{(k)} - \mathbf{W}^{(k)}\hat{\mathbf{x}}\|^2 
	\right ]
\end{equation}

\begin{gather}
	\frac{\mathrm{d}}{\mathrm{d} \hat{\mathbf{x}}}
	L\{\mathbf{y}^{(k)}\} = -\beta (\mathbf{y}^{(k)} - \mathbf{W}^{(k)}\hat{\mathbf{x}}) {\mathbf{W}^{(k)}}^T \\
	-\beta (\mathbf{y}^{(k)} - \mathbf{W}^{(k)}\hat{\mathbf{x}})^T \mathbf{W}^{(k)} = 0
\end{gather}
